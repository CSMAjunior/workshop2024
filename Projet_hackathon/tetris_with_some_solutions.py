...
#            self.fig_current_block = plt.figure()
    ...
    def find_max_y(self, b, ag):
        ...
        # pdb.set_trace()
        # if ymax < 0 or ymax-b.height[rot]+1 < 0:
    ...
    def print_game_state(self, plot_inline=True):     
        ...
        if plot_inline:
            ...
            plt.subplot(1,2,2)
            # self.print_block(self.block_list[self.current_bloc])
...
def get_reward_mean_line(current_state,new_state):
    meanLine = new_state.grid.shape[0] - np.mean(np.nonzero(new_state.grid-current_state.grid)[0])
    return meanLine

def get_reward_space(current_state,new_state):
    
    ### On évite d'augmenter les lignes occupées
    if len(np.nonzero(current_state.grid)[0]) == 0:
        size_c = 0
    else:
        size_c = np.nonzero(current_state.grid)[0].max() - np.nonzero(current_state.grid)[0].min()
    size_n = np.nonzero(new_state.grid)[0].max() - np.nonzero(new_state.grid)[0].min()
      
    # J'adimensionne par rapport au nombre de ligne total
    return (size_n-size_c) / new_state.grid.shape[0]
    
def get_reward_hole(current_state,new_state):
    ### On évite de laisser des trous sous le blocs
    # On récupère la position du bloc dans la grille par différence avec l'état antérieur
    
    H = new_state.grid.shape[0]
    pos_block = np.nonzero(new_state.grid - current_state.grid)
    # On boucle sur les colonnes correspondantes pour déterminer le nombre de trous au bas du bloc
    nb_trou = 0
    y_max_col = {col : H-1 for col in pos_block[1]}
    for ii,col in enumerate(pos_block[1]):
        if pos_block[0][ii] < y_max_col[col]:
            y_max_col[col] = pos_block[0][ii]
    
    for col in y_max_col.keys():
        nb_trou += H-y_max_col[col]-len(np.nonzero(new_state.grid[y_max_col[col]:,col])[0])
    # J'adimensionne par rapport au nombre total de case 
    return nb_trou / np.prod(new_state.grid.shape)

def get_reward_hole_better(current_state,new_state):
    ### On évite de laisser des trous sous le blocs, cette fois ci le test n'est pas jusqu'en bas mais
    ### seulement un peu en dessous du bloc
    # On récupère la position du bloc dans la grille par différence avec l'état antérieur
    ...
    for col in y_max_col.keys():
        minSize = min(H-1-y_max_col[col],len(np.unique(pos_block[1])))
        nb_trou += H-y_max_col[col]-len(np.nonzero(new_state.grid[y_max_col[col]:y_max_col[col]+minSize,col])[0])
    # J'adimensionne par rapport au nombre total de case
    return nb_trou / np.prod(new_state.grid.shape)
...
# nb_trou = G.L*G.H - len(np.nonzero(G.grid)[0])
# taux_remplissage = 1-nb_trou/(G.L*G.H)
